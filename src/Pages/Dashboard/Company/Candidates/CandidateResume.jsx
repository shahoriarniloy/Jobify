...
const CandidateResume = () => {
  ...
  return (
    <div>
      <div
        className="relative resume-container  py-10 px-16 shadow-lg bg-white rounded-lg   w-full mx-auto border-2"
        ref={contentRef}
      >
        <h1 className="text-3xl font-semibold text-blue-700">{resumeData?.name}</h1>
        <p className="text-xl text-gray-800 italic">{jobTitle}</p>

        <div className="contact-section my-3">
          <p>
            <FaEnvelope className="inline mr-2 text-blue-700" /> {resumeData?.email}
          </p>
          <p>
            <FaPhone className="inline mr-2 text-blue-700" /> {resumeData?.phone}
          </p>
          <p>
        ...
        <h2 className="font-bold mb-2 mt-4">
          <FaBriefcase className="inline mr-2 text-blue-700" /> {t("Experience")}
        </h2>
        ...
        <h2 className="font-bold mt-4 mb-2">
          <FaProjectDiagram className="inline mr-2 text-blue-700" /> {t("projects")}
        </h2>
        {resumeData.projects.map((project, index) => (
          <div key={index} className="my-2">
            <h3 className="font-bold italic">{project?.title}</h3>
            <p>
              {t("link")}: <a className="text-blue-600" href={project?.link}>{project?.link}</a>
            </p>
            <p className="text-gray-800 text-[15px]">{t("description")}: {project?.description}</p>
            <p className="text-gray-800 text-[15px]">{t("technologies")}: {project?.technologies}</p>
          </div>
        ))}

        <h2 className="font-bold mt-4 mb-2">
          <FaGraduationCap className="inline mr-2 text-blue-700" /> {t("education")}
        </h2>
        <p className="font-semibold italic text-gray-800">{resumeData?.education[0]?.schoolName}</p>
        <p className="text-sm text-gray-700 italic">{resumeData.education[0].degree}</p>
        <p className="text-sm text-gray-700">{t("year_of_graduation")}: {resumeData?.education[0]?.endDate}</p>
