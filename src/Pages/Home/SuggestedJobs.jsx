          ...
          {jobs?.slice(0, 5)?.map(({ _id, jobInfo, companyInfo }) => (
            ...
            >
              ...
              <div className="flex mt-4 md:mt-0 items-center gap-2 md:gap-3 self-stretch md:self-auto">
                <button className="btn bg-transparent border-none outline-none shadow-none hover:bg-[#e8f1ff] hover:text-[#0a65cc] p-2 md:p-3">
                  <Bookmark jobId={_id} />{" "}
                </button>

                <Link to={`/job/${_id}`}>
                  <button className="btn rounded-md md:rounded-none bg-[#e7f0fa] text-[#0a65cc] hover:bg-[#0a65cc] hover:text-white px-4 py-2 md:px-6 md:py-3 flex items-center gap-1 text-sm md:text-base">
                  {t('apply_now')}
                    <FaArrowRight />
                  </button>
                </Link>
