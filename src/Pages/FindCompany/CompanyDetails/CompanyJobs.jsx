  ...
  <div className="flex justify-center lg:flex-row flex-col mt-6">
    <div className="container mx-auto">
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-2 grid-cols-1 gap-12">
        {jobs.map((job, index) => (
        <div key={job._id} className={`border rounded-lg p-4 shadow-lg bg-white transition-transform duration-300
          ...
          <ApplyJobModal isOpen={isModalOpen} onClose={closeModal} job={job} />
          <div className="flex gap-3">
            <button onClick={openModal} className={`flex items-center justify-center gap-3 w-40 px-4 py-2 rounded-md
              font-semibold text-white ${ new Date()> new Date(job.deadline)
