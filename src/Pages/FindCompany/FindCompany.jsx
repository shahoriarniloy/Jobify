...
if (currentPage < pages.length - 1) { setCurrentPage(currentPage + 1); } }; return ( <div className=" pt-24 ">
  <div className="w-full bg-white rounded-lg  p-6 flex-1">
    <form className="flex flex-col sm:flex-row gap-4 sm:gap-2" onSubmit={handleSearch}>
      <div className="relative flex-1">
        ...
        <input type="text" placeholder="Job title, Keyword..." value={searchTerm} onChange={(e)=>
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full text-xs sm:text-sm">
          ...
          <tbody>
            ...
            <tr key={company._id} className="border-b border-opacity-20 dark:border-gray-300 dark:bg-gray-50">
              <td className="p-3">
  ...
  <div className="grid grid-cols-1 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-4  mt-4 gap-6">
    {(filteredCompanies.length > 0 ? filteredCompanies : companies).map((company) => (
    <div key={company._id} className="max-w-xs rounded-md shadow-md dark:bg-gray-50 dark:text-gray-800">
  ...
  <div className="p-4 flex justify-center items-center">
    <button className="px-4 py-2 mr-2 bg-blue-600 text-white rounded-lg shadow hover:bg-blue-700"
      onClick={handlePreviousPage} disabled={currentPage===0}>
      Prev
    </button>
    <div className="flex gap-2">
      {pages.map((page) => (
      <button key={page} className={`px-4 py-2 rounded-lg ${ page===currentPage ? "bg-blue-200" : "bg-white" } border
        border-blue-300`} onClick={()=> setCurrentPage(page)}
        >
        {page + 1}
      </button>
      ))}
    </div>
    <button className="px-4 py-2 ml-2 bg-blue-600 text-white rounded-lg shadow hover:bg-blue-700"
      onClick={handleNextPage} disabled={currentPage===pages.length - 1}>
      Next
    </button>
  </div>
